...
alldata = []
diff_token='c751edf1f12a5389944c359a95f92271'
scraped_urls = set()
...
def read_excel(filename, start):
    ...
    for i in range(start, table.nrows):
        try:
            if i not in [4,5,11,21,22,26,27,29,30,33,38,40,41,43,45,46,47,49]:
                continue
            url = table.row(i)[1].value.strip()
...
def read_scraped_excel(filename, start):
    global scraped_urls
    data = xlrd.open_workbook(filename)
    table = data.sheets()[0]
    for i in range(start, table.nrows):
        try:
            url = table.row(i)[0].value.strip()
...
# read_scraped_excel('data/sheet3.xlsx', 0)
read_excel('data/sheet1.xls', 1)
write_excel('data/sheet3_1.xlsx', alldata)