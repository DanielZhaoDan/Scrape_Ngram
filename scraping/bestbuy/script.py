...
base_url = 'https://www.bestbuy.com/site/searchpage.jsp?_dyncharset=UTF-8&id=pcat17071&iht=y&keys=keys&ks=960&list=n&sc=Global&sp=-bestsellingsort%20skuidsaas&st=Intel%20laptop&type=page&usc=All%20Categories&cp='
...
def request_sheet1(page_no):
    ...
    reg = 'class="list-item lv" data-sku-id="(.*?)".*?class="image-link" href="(.*?)".*?src="(.*?)".*?right-column(.*?)class="sku-header">(.*?)</h4.*?c-ratings-reviews-v2.*?sr-only">(.*?)<'
    ...
    for i in range(len(data_list)):
        ...
        one_row = [prefix + id, item_url, img_url, is_sponsored, is_new, title, rating, no_review]
...
utils.write_excel("data/sheet2.xls", sheet2)
driver.close()