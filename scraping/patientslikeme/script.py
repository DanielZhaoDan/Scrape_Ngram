...
sheet1_data = [['Topic', 'Topic URL', 'No. Likes', 'No. Replies', 'Last Reply Date', 'Last Reply content']]
sheet2_data = [['Topic', 'Topic URL', 'Reply Date', 'Reply Content']]

url_base = 'https://www.patientslikeme.com/forum/lungs_respiratory/topics?page=%d&prompt=condition_infos_any'
...
def request_sheet1(url):
    ...
    topic_detail_reg = 'sub-title has-tooltip has-tooltip-right icon-spacer" href="(.*?)">(.*?)<.*?Number of comments:<.*?>(.*?)<.*?</span>(.*?)<.*?"last-reply".*?class="date"><a.*?>(.*?)<.*?class="helptext">(.*?)<'
    topic_detail = re.compile(topic_detail_reg).findall(html)
    for detail in topic_detail:
        topic_url = 'https://www.patientslikeme.com' + detail[0]
        topic = detail[1]
        replied = int(detail[2])
        likes = int(detail[3])
        last_date = detail[4]
        last_content = detail[5]

        one_row = [topic, topic_url, likes, replied, last_date, last_content]
        sheet1_data.append(one_row)
        if replied == 0:
            continue
        page_number = replied / 20 + 1
        request_sheet2(topic, topic_url+'?page=%d&prompt=condition_infos_any',  page_number)


def request_sheet2(topic, url2_base, number):
    global sheet2_data
    reg = '<time datetime="(.*?)".*?class="text">(.*?)</div>'
    for i in range(1, number+1):
        ...
        for reply in reply_lists:
            ...
            one_row = [topic, url2_base.split('?')[0], date, content]
